...
timeline = [
    (0.35, 8.6, 1.5, "Pod 申请挂载", "#aec7e8"),
    (3.0, 8.6, 2.7, "Vault 创建租约\n生成 user_xxx / pwd_yyy", "#2ca02c"),
    (6.3, 8.6, 2.3, "TTL 到期后文件保持陈旧值\nPod 重建时重新签发", "#ff7f0e"),
    (9.0, 8.6, 2.1, "max_ttl 强制过期\n或 Pod 删除 -> revoke", "#d62728"),
]
for cx, cy, w, txt, c in timeline:
    ax2.add_patch(mpatches.FancyBboxPatch((cx - w / 2, cy - 0.65),
                  w, 1.3, boxstyle="round,pad=0.08", fc=c, ec="black", alpha=0.92))
    ax2.text(cx, cy, txt, ha="center", va="center",
             fontsize=8.8, color="white")
ax2.annotate("", xy=(1.61, 8.6), xytext=(1.14, 8.6), arrowprops=dict(arrowstyle="-|>", lw=1.6))
ax2.annotate("", xy=(5.11, 8.6), xytext=(4.39, 8.6), arrowprops=dict(arrowstyle="-|>", lw=1.6))
ax2.annotate("", xy=(7.91, 8.6), xytext=(7.49, 8.6), arrowprops=dict(arrowstyle="-|>", lw=1.6))
...
ax2.set_xlim(-0.65, 10.15); ax2.set_ylim(0, 9.6); ax2.axis("off")
