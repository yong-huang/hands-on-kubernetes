...
![RBAC 模型](images/rbac_model.svg)

模型主线：Subject（SA 是进程身份，User/Group 来自外部认证）经 RoleBinding 的 `subjects` 绑定 → `roleRef` 引用 Role 的 rules（apiGroups/resources/verbs）→ API Server 鉴权时**任一规则匹配即放行，全不匹配返回 403**。权限只加不减、默认拒绝，所以粒度宁细勿粗。
