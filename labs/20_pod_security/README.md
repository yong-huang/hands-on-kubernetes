...
└── images/
       └── pod_security_arch.png # 三等级策略 + 准入流程示意图
```
...
左图是 PSS 三等级金字塔（每级禁什么）；右图是 PSA 准入流程：Pod 创建 → 读 namespace 标签 → enforce 拒绝 / audit 留痕 / warn 提示 / 合规放行，以及标签解剖：

![pod_security](images/pod_security_arch.png)
