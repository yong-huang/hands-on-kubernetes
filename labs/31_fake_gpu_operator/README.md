...
└── images/
    └── fake_gpu_operator_arch.png   # 架构图（gen_arch.py 生成）
```
...
![fake gpu](images/fake_gpu_operator_arch.png)

上图两面板：
- **左图 设备插件链路**：真实链路（driver → gpu-operator → gRPC 上报）与模拟链路（patch status 字段）殊途同归于 Node Status；强调调度器只认数字不辨真伪
- **右图 调度关卡**：Job 提交后依次过 ResourceQuota 和 Scheduler Filter 两关，small/big 两个任务的两种结局；附巡检命令速查
