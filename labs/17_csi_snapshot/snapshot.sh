...
# =============================================================================
# CSI 快照与还原演示脚本
# 覆盖: 装 CRD/controller -> 部署源数据 -> 创建快照 -> 诚实验证 -> 清理
# 用法: ./snapshot.sh [step]   (crd | deploy | snapshot | verify | clean | all)
#
...
driver_of_pvc() {
    kubectl get pvc "${PVC}" -n "${NAMESPACE}" \
        ...
        | xargs -r kubectl get sc -o jsonpath='{.provisioner}'
}
...
# ----------------------------- 2. 部署源数据 -----------------------------
do_deploy() {
    step "deploy" "创建 PVC + Pod, 并写入将被快照的标记数据"
    kubectl apply -f manifests/csi_snapshot.yaml
...
# ----------------------------- 3. 创建快照并诚实轮询 -----------------------------
do_snapshot() {
    step "snapshot" "创建 VolumeSnapshotClass + VolumeSnapshot"
    kubectl apply -f manifests/csi_snapshot.yaml
...
do_verify() {
    ...
    if [[ "${ready}" == "true" ]]; then
        step "verify" "支持的驱动上: 创建还原 PVC 并对比数据"
        kubectl apply -f manifests/csi_snapshot.yaml          # 含 dataSource 的还原 PVC
        ...
        cat <<'EOF'
还原清单 (csi_snapshot.yaml 第 4 段, 在支持快照的驱动上 apply 即可):
...
main() {
    ...
    case "${target}" in
        ...
        verify)   do_verify ;;
        clean)    do_clean ;;
        all)      do_crd; do_deploy; do_snapshot; do_verify; do_clean ;;
        *)
            echo "未知步骤: ${target}" >&2
            echo "可用: crd | deploy | snapshot | verify | clean | all" >&2
            exit 1 ;;
    esac
}
main "$@"
